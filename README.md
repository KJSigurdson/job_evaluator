...
**Both tiers run concurrently**, each bounded by its own env var — `TIER1_CONCURRENCY` (`src/scoring.py`'s `score_many`/`ascore`/`_acall_llm`) and `TIER2_CONCURRENCY` (`src/enrich.py`'s `enrich_many`/`aenrich`/`_acall_llm`) — both default 10, both driven via `AsyncAnthropic`. Tier 1 runs at roughly 6 seconds per call and Tier 2 (Sonnet) at roughly 25 seconds; sequentially, either one dominates wall-clock time for a large run in turn — a 1,436-gate-survivor run once finished Tier 1 in under a minute but was still 38 matches into sequential Tier 2 enrichment after 16 minutes when cancelled. Running many postings/enrichments under a bounded semaphore instead keeps wall-clock time proportional to `count / CONCURRENCY` rather than `count` outright. The final matches-upsert/seen-verdict-queuing/digest-bookkeeping step for each Tier 2 result stays sequential — those are cheap Supabase writes, not the bottleneck, and don't need concurrency. Concurrency inevitably produces `429`s the old one-shot parse-retry wasn't built for, so each tier's `_acall_llm` wraps the actual API call in its own retry loop specifically for `RateLimitError` — up to 5 attempts, exponential backoff with jitter, logged at INFO (not WARNING/ERROR — expected, self-resolving backoff shouldn't page anyone). A `RateLimitError` that survives all 5 backoff attempts is just a normal exception to the existing one-shot retry-then-`ScoringError`/`EnrichmentError` wrapper, so it's still handled the same way a parse failure always was — no new failure mode. The safe concurrency ceiling for each depends on the Anthropic account's rate-limit tier, which isn't discoverable from the codebase: start at the defaults of 10 and watch the log for "rate-limited" lines; raise or lower per-tier based on how often they fire. The original sequential `score()`/`_call_llm()`/`_invoke_with_retry()` and `enrich()`/`_call_llm()` are untouched — they're what the test suite injects/monkeypatches against — and remain available for anything that still wants single-posting scoring or enrichment.

**Tool-use responses that span multiple content blocks are merged, not truncated to the first one.** A real run surfaced a Tier 1 response split across seven separate `tool_use` blocks (one scoring key each) instead of one block with all seven — `response.content[0].input` alone only saw the first fragment and failed validation identically on both retry attempts, a permanent, silent loss of that posting. Both tiers now collect every `tool_use` block matching the expected tool name and merge their `.input` dicts before validating (`_merge_tool_use_blocks`, duplicated in `scoring.py` and `enrich.py` — this codebase keeps per-module private helpers self-contained rather than introducing a shared utils module for one small function). A response with zero matching blocks still raises the same error path as before; this is additive robustness, not a relaxation of failure handling.
...
src/
  ...
  enrich.py           Tier 2 (Sonnet) enrichment — sync enrich()/_call_llm() unchanged (plus the multi-block merge fix and defensive list-coercion for CV guidance); additive concurrent path (enrich_many/aenrich/_acall_llm) used by pipeline.py
